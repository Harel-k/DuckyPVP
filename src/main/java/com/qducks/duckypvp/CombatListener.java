...
public final class CombatListener implements Listener {
    ...
    public CombatListener(DuckyPVP plugin, ArenaManager arena, CombatManager combat) {
        this.plugin = plugin;
        this.arena = arena;
        this.combat = combat;
    }
    ...
    public void onJoin(PlayerJoinEvent event) {
        plugin.getServer().getScheduler().runTask(plugin, () -> {
            if (arena.isInArena(event.getPlayer().getLocation())) {
                combat.onArenaEnter(event.getPlayer());
            }
        });
    }
